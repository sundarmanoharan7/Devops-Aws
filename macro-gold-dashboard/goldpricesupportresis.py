...
st.title("Gold (XAUUSD) Structural Support & Resistance")

# --- DATA FETCHING ---
@st.cache_data(ttl=3600)
def fetch_gold_data(period="30d"):
    # Fetch GC=F futures for robust historical liquidity
...
if not df_1h.empty:
    ...
    # Current active close price
    live_price = float(df_4h['Close'].iloc[-1])
    
    # Isolate closest resistance above and supports below
    ...
    if sup_minor: 
        st.write(f"Minor Support: ${sup_minor:,.2f}")
    if sup_maj1: 
        st.write(f"Major Support 1: ${sup_maj1:,.2f}")
    if sup_maj2: 
        st.write(f"Major Support 2: ${sup_maj2:,.2f}")
